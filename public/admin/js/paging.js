...
layui.define(['layer', 'laypage', 'laytpl'], function (exports) {
     ...
     * @param {Object} options
     */
    Paging.prototype.rget = function (options) {
        var that = this;
        var _config = that.config;
        ...
        //默认参数
        var df = {
            pageIndex: _config.InitPageIndex,
            pageSize: _config.pageConfig.pageSize
        };
                    ...
                    if (_config.paged) {
                        ...
                        var defaults = {
                            ...
                            jump: function (obj, first) {
                                //得到了当前页，用于向服务端请求对应数据
                                var curr = obj.curr;
                                var floorPages = parseInt(pages);//小数取整
                                (floorPages === pages) && (floorPages += 1);//如果相等，自增1
                                if (result.total === 1 && result.total === 0) {
                                    that.rget({
                                        pageIndex: _config.InitPageIndex - 1,
                                        pageSize: pageSize
                                    });
                                }
                                _config.InitPageIndex = floorPages;
                    ...
                    if (_config.paged) {
                        ...
                        var defaults = {
                            ...
                            jump: function (obj, first) {
                                ...
                                //var floorPages=Math.floor(pages); //小数取整
                                var floorPages = parseInt(pages);//小数取整
                                (floorPages === pages) && (floorPages += 1);//如果相等，自增1

                                    if (result.total===0){

                                        if(_config.InitPageIndex===1){
